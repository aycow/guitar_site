...
// Extend the base type to include avatarColor returned from our updated scores API
...
function PlayerAvatar({ name, color, avatarUrl, size = 36 }: {
  name: string;
  color?: string;
  avatarUrl?: string | null;
  size?: number;
  ...
  return (
    <div style={{
      ...
      fontSize: size * 0.38, fontWeight: 900, color: "#000",
...
export default function LeaderboardPage() {
  const [leaderboard, setLeaderboard] = useState<LeaderboardEntryExtended[]>([]);
  ...
  async function fetchLeaderboard() {
    try {
      ...
      setLeaderboard(data.entries as LeaderboardEntryExtended[]);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <nav className="border-b border-[#1f2937] bg-[#0d1117]/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-4">
            <Link href="/game/g_scale"><Button variant="ghost">Play</Button></Link>
      ...
      <div className="mx-auto max-w-7xl px-6 py-12">
        ...
        {/* ── Header ── */}
        <div className="mb-10">
          <h2 className="text-4xl font-bold text-white mb-3 font-mono tracking-wide">🏆 Global Leaderboard</h2>
          <p className="text-zinc-400 mb-8">Click any player to view their full profile.</p>

          <div className="flex gap-4 flex-wrap items-end">
            <div>
              <label className="block text-sm font-semibold text-zinc-300 mb-2">Filter by Level</label>
              <input
                type="text"
        ...
        {!loading && leaderboard.length >= 3 && (
          <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
            {[leaderboard[1], leaderboard[0], leaderboard[2]].map((entry, podiumIdx) => {
              ...
              const colors: Record<number, string> = { 1: "#facc15", 2: "#9ca3af", 3: "#f97316" };
              ...
              return (
                ...
                >
                  ...
                  onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.transform = "translateY(-3px)"; (e.currentTarget as HTMLDivElement).style.borderColor = accentColor; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.transform = ""; (e.currentTarget as HTMLDivElement).style.borderColor = `${accentColor}40`; }}
                  >
                    {/* Glow top */}
                    <div style={{
                    ...
                    <div style={{ fontSize: 28, marginBottom: 10 }}>{rankBadge[entry.rank]}</div>
                    <div style={{ display: "flex", justifyContent: "center", marginBottom: 10 }}>
                      <PlayerAvatar name={entry.playerName} color={entry.avatarColor} avatarUrl={entry.avatarUrl} size={isFirst ? 56 : 44} />
                    </div>
                    <div style={{ fontWeight: 700, fontSize: isFirst ? 17 : 15, color: "#f0f0f0", marginBottom: 4 }}>
                      {entry.playerName}
                    </div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: accentColor, marginBottom: 2 }}>
                      {entry.score.toLocaleString()}
                    </div>
                    <div style={{ fontSize: 12, color: grade.color }}>{entry.accuracy.toFixed(1)}% acc</div>
                  </div>
                </Link>
              );
            })}
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      background: isHovered
                        ? "#1f2937"
                        : index % 2 === 0 ? "transparent" : "#ffffff03",
                      alignItems: "center",
                      cursor: "pointer",
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {isTop3 ? (
                        <span style={{ fontSize: 20 }}>{rankBadge[entry.rank]}</span>
                      ) : (
                        <span style={{
                          width: 30, height: 30, borderRadius: "50%",
                          background: "#1f2937", display: "flex", alignItems: "center",
                          justifyContent: "center", fontSize: 12, color: "#6b7280", fontWeight: 700,
                        }}>
                          {entry.rank}
                        </span>
                      )}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div>
                        <div style={{
                          fontWeight: 700, fontSize: 14, color: isHovered ? "#22c55e" : "#f0f0f0",
                          transition: "color 0.1s",
                        }}>
                          {entry.playerName}
                        </div>
                        {isHovered && (
                          <div style={{ fontSize: 10, color: "#4b5563", letterSpacing: 1, marginTop: 1 }}>
                            VIEW PROFILE →
                          </div>
                        )}
                      </div>
                    ...
                    {/* Score */}
                    <span style={{ color: "#facc15", fontWeight: 700, fontSize: 16 }}>
                      {entry.score.toLocaleString()}
                    </span>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ flex: 1, height: 4, background: "#1f2937", borderRadius: 2, overflow: "hidden" }}>
                        <div style={{
                          height: "100%", width: `${entry.accuracy}%`,
                          background: `linear-gradient(90deg, ${grade.color}80, ${grade.color})`,
                          borderRadius: 2,
                        }} />
                      </div>
                      <span style={{ color: grade.color, fontWeight: 700, fontSize: 13, minWidth: 44, textAlign: "right" }}>
                        {entry.accuracy.toFixed(1)}%
                      </span>
                    </div>

                    {/* Hits */}
                    <span style={{ color: "#22c55e", fontSize: 13 }}>{entry.hits}</span>

                    {/* Misses */}
                    <span style={{ color: "#ef4444", fontSize: 13 }}>{entry.misses}</span>
        ...
        {!loading && leaderboard.length === 0 && !error && (
          <div className="bg-zinc-900/50 border border-zinc-700 rounded-xl p-12 text-center">
            <p className="text-zinc-400 text-lg mb-6">No scores yet — be the first!</p>
            <Link href="/game/g_scale"><Button size="lg">Start Playing</Button></Link>
          </div>
        )}
        ...
        {!loading && leaderboard.length > 0 && (
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { label: "Total Players",  value: leaderboard.length,              color: "emerald" },
              { label: "Top Score",      value: topScore.toLocaleString(),        color: "cyan"    },
              { label: "Avg Accuracy",   value: `${avgAccuracy}%`,               color: "amber"   },
            ].map(({ label, value, color }) => (
              <div key={label} className={`bg-gradient-to-br from-${color}-500/10 to-${color}-600/5 border border-${color}-500/30 rounded-xl p-6`}>
